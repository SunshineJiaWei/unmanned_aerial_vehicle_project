...
返回  值：状态寄存器的值
*********************************************************/
uint8_t int_si24r1_SI24R1_WRITE_REG(uint8_t reg, uint8_t value)
{
	uint8_t status;
...
返回  值：寄存器值
*********************************************************/
uint8_t int_si24r1_SI24R1_READ_REG(uint8_t reg)
{
	uint8_t value;
...
void int_si24r1_rx_mode(void)
{
	...
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + EN_AA, 0x01);							// 使能接收通道0自动应答
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + EN_RXADDR, 0x01);						// 使能接收通道0
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + RF_CH, TX_RX_CHANNEL);					// 选择射频通道0x40
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + RX_PW_P0, TX_PLOAD_WIDTH);				// 接收通道0选择和发送通道相同有效数据宽度
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + RF_SETUP, 0x0f);						// 数据传输率1Mbps，发射功率4dBm
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + CONFIG, 0x0f);							// CRC使能，16位CRC校验，上电，接收模式
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + STATUS, 0xff);							// 清除所有的中断标志位
	CE_HIGH();																// 拉高CE启动接收设备
}
...
void int_si24r1_tx_mode(void)
{
	...
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + EN_AA, 0x01);			// 使能接收通道0自动应答
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + EN_RXADDR, 0x01);		// 使能接收通道0
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + SETUP_RETR, 0x0a);		// 自动重发延时等待250us+86us，自动重发10次
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + RF_CH, TX_RX_CHANNEL); // 选择射频通道0x40
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + RF_SETUP, 0x0f);		// 数据传输率2Mbps，发射功率7dBm
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + CONFIG, 0x0e);			// CRC使能，16位CRC校验，上电
	CE_HIGH();												// CE = 1;
}
...
{
	uint8_t state;
	state = int_si24r1_SI24R1_READ_REG(STATUS);			 // 读取状态寄存器的值
	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + STATUS, state); // 清除RX_DS中断标志

	if (state & RX_DR) // 接收到数据
	{
		int_si24r1_read_buf(RD_RX_PLOAD, rxbuf, TX_PLOAD_WIDTH); // 读取数据
		int_si24r1_SI24R1_WRITE_REG(FLUSH_RX, 0xff);					 // 清除RX FIFO寄存器
		return 0;
	}
	return 1; // 没收到任何数据
}
...
uint8_t int_si24r1_tx_packet(uint8_t *txbuf)
{
	...
	{
		state = int_si24r1_SI24R1_READ_REG(STATUS);
		vTaskDelay(1); // 短暂让出CPU，防止通讯失败时，导致程序一直卡在这
	}

	int_si24r1_SI24R1_WRITE_REG(SI24R1_WRITE_REG + STATUS, state); // 清除TX_DS或MAX_RT中断标志
	if (state & MAX_RT)								 // 达到最大重发次数
	{
		int_si24r1_SI24R1_WRITE_REG(FLUSH_TX, 0xff); // 清除TX FIFO寄存器
		return 1;
	}
	if (state & TX_DS) // 发送完成
	{
